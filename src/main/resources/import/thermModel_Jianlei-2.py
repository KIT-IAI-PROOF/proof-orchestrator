...
class Program:
    ...
    def step(self):
        ...
        print("communication_step_size", self.communication_step_size)
        print("c", self.c)
        print("m", self.m)
        print(self.T_therm - self.T_ambient)
        Q = 3 * 0.00314 * (self.T_therm - self.T_ambient)
        print("Q", Q)
        power = self.power_loss_therm - Q
        print("power", power)
        dT = power * self.communication_step_size / (self.c * self.m)
        print("dT", dT)
        self.T_therm += dT
    ...
    def main(self) -> None:
        print("thermModel-2 main")
        while True:
            print("thermModel-2 readline")
            line = stdin.readline()
            print("thermModel-2 after readline")
            message = loads(line)
            message_type = message["type"]
            if message_type == "VALUE":
                print("THERM-2 PROGRAM RECEIVED VALUEMESSAGE={0}".format(message), flush=True)
                message_data = message["data"]
                self.set_data(message_data)
            elif message_type == "SYNC":
                print("THERM-2 PROGRAM RECEIVED SYNCMESSAGE={0}".format(message), flush=True)
