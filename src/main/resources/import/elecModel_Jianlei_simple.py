...
class Program:
    ...
    def step(self):
        ...
        if self.communicationPoint + 1 < self.tmax:
            if self.communicationPoint == 0:
                ...
                print("Nothing here, because the first step is over!!!")
                # while True:
                #     if self.T != 0.0:
            ...
            print("Nothing to do, because the current communicationPoint + 1 ", self.communicationPoint, " >= ",
                  self.tmax, "(tmax)!!!")
            self.finalize()
        self.get_data(1)
        self.notify(1)
    ...
    def main(self) -> None:
        print("elecModel main")
        while True:
            print("elecModel readline")
            line = stdin.readline()
            print("elecModel after readline")
            message = loads(line)
            message_type = message["type"]
            if message_type == "VALUE":
                print("ELEC PROGRAM RECEIVED VALUEMESSAGE={0}".format(message), flush=True)
                message_data = message["data"]
                self.set_data(message_data)
            elif message_type == "SYNC":
                print("ELEC PROGRAM RECEIVED SYNCMESSAGE={0}".format(message), flush=True)
